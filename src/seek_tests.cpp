...
TEST(SeekTest, EmptySlice) {
    ...
    auto result = join::seek(s, [](const int &x) { return x < 5; });
...
TEST(SeekTest, AllElementsMatch) {
    ...
    std::span<const int> s(v);
    auto result = join::seek(s, [](const int &x) { return x < 100; });
...
TEST(SeekTest, SomeElementsMatch) {
    ...
    std::span<const int> s(v);
    auto result = join::seek(s, [](const int &x) { return x < 10; });
    EXPECT_EQ(result.size(), 3);
...
TEST(SeekTest, ExponentialGallopTriggered) {
    ...
    for (int i = 0; i < 1000; ++i)
        v.push_back(i);
    std::span<const int> s(v);
    auto result = join::seek(s, [](const int &x) { return x < 500; });
