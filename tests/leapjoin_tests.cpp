...
TEST(LeapjoinTest, SingleExtendWith) {
    ...
    auto result = leapjoin<int>(source, collection, [](int prefix, int val) {
        return std::pair{prefix, val};
    });

    EXPECT_EQ(result.elements,
              (std::vector<std::pair<int, int>>{{1, 10}, {1, 20}, {2, 30}}));
}
...
TEST(LeapjoinTest, WithPrefixFilter) {
    ...
    auto ew = rl.extend_with<int>([](int x) { return x; });
    auto pf = filters::prefix_filter<int>([](int x) { return x <= 2; });
...
TEST(LeapjoinTest, WithPassthrough) {
    ...
    std::vector<int> source = {1, 2, 3};
    auto result =
        leapjoin<int>(source, collection,
