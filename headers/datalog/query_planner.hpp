...
template <typename Atoms> constexpr auto level_plan(int s, int K) {
    constexpr size_t NA = std::tuple_size_v<Atoms>;
    constexpr size_t NV = num_vars<Atoms>();
    constexpr auto ids = atom_ids<Atoms>();
    constexpr auto arities = atom_arities<Atoms>();
    auto pos = invert<NV>(make_order<Atoms>(s));
    LevelPlan<NA> lp{};
    for (size_t a = 0; a < NA; ++a) {
        if ((int)a == s)
            continue;
...
template <int S, typename Atoms, typename Filters>
...
auto make_ext(const Atoms &atoms, const std::array<int, NV> &pos) {
    constexpr auto ids = atom_ids<Atoms>();
    constexpr auto arities = atom_arities<Atoms>();
...
df::Relation<std::array<V, NV>> extend(df::Relation<std::array<V, K>> prefix,
    ...
    if constexpr (K == NV) {
        ...
        constexpr auto lp = level_plan<Atoms>(S, (int)K);
        auto exts = build_exts<V, K, S, (int)K, Atoms>(
...
df::Relation<std::array<V, NV>> extend(std::span<const std::array<V, K>> src,
    ...
    if constexpr (K == NV) {
        ...
        auto exts = build_exts<V, K, S, (int)K, Atoms>(
            atoms, std::make_index_sequence<lp.n>{});
        auto coll = to_coll<V, K>(std::move(exts));
        auto next = df::leapjoin(src, coll, ArrayAppender<V, K>{});
        return extend<V, NV, S, K + 1>(std::move(next), atoms);
    }
}
...
struct QueryPlanner {
    ...
    template <int S> static constexpr bool source_is_forward_viable() {
        ...
        for (int K = (int)src_ar; K < (int)NV; ++K)
    ...
    void operator()() const {
        for_indices<NA>([&]<size_t S>() { do_source<(int)S>(); });
    }

    void eval_full() const {
        for_indices<NA>([&]<size_t S>() { do_source_full<(int)S>(); });
    }
    ...
    template <int S> void do_source_full() const {
        ...
        if constexpr (source_is_forward_viable<S>()) {
            auto *sp = std::get<S>(atoms).pred;
            for (const auto &batch : sp->var.stable)
                do_source_impl<S>(
    ...
    static constexpr bool atom_needs_semijoin_check() {
        ...
        constexpr auto cpos = invert<NV>(make_order<Atoms>(S));
        bool forward = true;
        for (size_t c = 1; c < ar; ++c)
    ...
    bool filter_check(const std::array<V, NV> &a,
        ...
        constexpr int ia = filter_vars<Filt>::a_id;
        constexpr int ib = filter_vars<Filt>::b_id;
        ...
        if constexpr (is_negated<Filt>::value) {
            return !std::get<F>(filters).pred->stable_contains({a[pa], a[pb]});
        } else {
            constexpr Cmp op = filter_vars<Filt>::op;
            return cmp_apply<op>(a[pa], a[pb]);
        }
    }
