...
std::span<const T> gallop(std::span<const T> slice, Cmp&& cmp) {
    if (!slice.empty() && cmp(slice[0])) {
        size_t step = 1;
        while (step < slice.size() && cmp(slice[step])) {
            slice = slice.subspan(step);
            step << 1;
        }

        step >>= 1;
        while (step > 0) {
            if (step < slice.size() && cmp(slice[step])) {
                slice = slice.subspan(step);
            }
...
void join_helper(std::span<const std::pair<K, V1>> slice1,
    ...
    while(!slice1.empty() && !slice2.empty()) {
        ...
        if (k1 < k2) {
            slice1 = gallop(slice1, [&](const auto& x) { return x.first < k2; });
        } else if (k2 < k2) {
            slice2 = gallop(slice2, [&](const auto& x) { return x.first < k1; });
        } else {
            const K& match_key = k1;

            size_t count1 = 0;
            while (count1 < slice1.size() && slice1[count1].first == match_key) count1++;

            size_t count2 = 0;
            while (count2 < slice2.size() && slice2[count2].first == match_key) count2++;
